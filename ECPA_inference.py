...
from src.modeling.video_captioning_e2e_vid_swin_bert_share_fusion import VideoTransformer
...
# # from .ANFL import MEFARG
# from ANFL import MEFARG###
# net = MEFARG(num_main_classes=27, num_sub_classes=14, backbone='resnet50',
...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
 ...
 ###AU for里面多了一个aufeats参数\
    ###dwa
    avg_cost = np.zeros([100, 3], dtype=np.float32)
    lambda_weight = np.ones([3, 100])
    T = 2.0
    ###dwa
    for iteration, (img_keys, batch, meta_data, au_feats, au_res_feats) in enumerate(train_dataloader):
        iteration += 1
        # iteration = global_iters_per_epoch+1
        data_time = time.time() - end
        batch = tuple(t.to(args.device) for t in batch)
        au_feats = au_feats.to(device)
        au_res_feats =au_res_feats.to(device)
        model.train()
        ###dwa
        index = int(iteration / global_iters_per_epoch)
        index_avg = int(iteration % global_iters_per_epoch)

        if index_avg - 1 == 0:
            cost = np.zeros(3, dtype=np.float32)

        if index_avg - 1 == 0 and (index == 0 or index == 1):
            lambda_weight[0, index] = 1.0
            lambda_weight[1, index] = 1.0
            lambda_weight[2, index] = 1.0
        if index_avg - 1 == 0 and index > 1:
        ...
        inputs = {
            'input_ids': batch[0], 'attention_mask': batch[1],
            'token_type_ids': batch[2], 'img_feats': batch[3],
            'masked_pos': batch[4], 'masked_ids': batch[5], 'emo_label': batch[6],
            ###AU
            'au_feats':au_feats, ###2,9,512
            'au_res_feats':au_res_feats, ###2,9,512
            ###AU
            }
        ...
        if iteration == 1:
            for k, v in inputs.items():
                try: ###因为phrases是一个列表中装的两个dict 不是tensor 没有shape
                    logger.info(f'{k} = {v.shape}')
                except:
                    logger.info(f'{k}')   
        # inputs = fp32_to_fp16(inputs[0])
        ...
        if args.mixed_precision_method == "fairscale":
            ...
            outputs = model(**inputs)
        masked_loss, logits, emo_loss, emo_logits,au_loss  = outputs[:5]
        # print (logits)
        logits_cpu_detached = logits.cpu().detach()
        np.savetxt('logits.txt', logits_cpu_detached, delimiter='\t', fmt='%f')  # fmt='%f' 表示使用浮点数格式保存
        loss = masked_loss
        ...
        if args.learn_mask_enabled:
            ...
            ###AU
            loss = loss + (loss_sparsity * args.loss_sparse_w) + 0.6 * au_loss
        ...
        emo_label = inputs['emo_label']
        # batch_size = emo_label.size()

        ###dwa
        cost[0] += loss.item()
        cost[1] += emo_loss.item()
        cost[2] += au_loss.item()
        if index_avg == 0:
            avg_cost[index-1, :3] = cost[:3] / global_iters_per_epoch
        ###dwa

        # avg_cost[index, :3] += cost[:3] / batch_size

        batch_score = compute_score_with_logits(logits, masked_ids)
        batch_acc = torch.sum(batch_score.float()) / torch.sum(inputs['masked_pos'])
        # print(batch_score)
        # print(torch.sum(batch_score.float()))
        emo_batch_score = compute_score_with_logits(emo_logits, emo_label)
        emo_batch_acc = torch.sum(emo_batch_score.float()) / 2
        # print(emo_batch_score)
        # print(torch.sum(emo_batch_score.float()))

        if args.learn_mask_enabled:
            ###AU
            loss_dict = {'masked_loss': masked_loss, 'loss_sparsity': loss_sparsity.item(),'emo_loss':emo_loss, 'emo_acc':emo_batch_acc,'au_loss':au_loss, 'acc': batch_acc}
        else:
            ###AU
            # loss = loss + 0.6 * au_loss 
            loss_dict = {'weight_masked': lambda_weight[0,index], 'weight_cls': lambda_weight[1,index], 'weight_au': lambda_weight[2,index], 'masked_loss': masked_loss, 'acc': batch_acc,'emo_loss':emo_loss, 'emo_acc':emo_batch_acc,'au_loss':au_loss} # 
            ###AU

            ###dwa
            # loss = lambda_weight[0, index]*masked_loss + lambda_weight[1, index]*emo_loss + lambda_weight[2, index]*au_loss
            ###dwa
...
def test(args, test_dataloader, model, tokenizer, predict_file):
    ...
    def gen_rows():
        ...
        with torch.no_grad():
            ###AU for循环里的aufeats,phrase
            for step, (img_keys, batch, meta_data, au_feats,  au_res_feats) in tqdm(enumerate(test_dataloader)):
                # torch.cuda.empty_cache()
                is_exist = True
                ###new
                au_feats = au_feats.to(device)
...
def check_arguments(args):
    ...
    print(int((args.max_num_frames/2)))
...
def get_custom_args(base_config):
    ...
    parser.add_argument('--backbone_coef_lr', type=float, default=0.05)
